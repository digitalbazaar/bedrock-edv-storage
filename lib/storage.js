...
api.insertConfig = async ({actor, config}) => {
  ...
  try {
    const result = await database.collections.edvConfig.insert(
...
api.findConfig = async (
  ...
  return database.collections.edvConfig.find(
...
api.updateConfig = async ({actor, config}) => {
  ...
  const result = await database.collections.edvConfig.update({
    id: database.hash(config.id),
...
api.insert = async ({edvId, doc}) => {
  ...
  try {
    const result = await database.collections.edvDoc.insert(
      record, database.writeOptions);
    return result.ops[0];
  } catch(e) {
    if(!database.isDuplicateError(e)) {
...
api.find = async ({edvId, query = {}, fields = {}, options = {}}) => {
  ...
  return database.collections.edvDoc.find(query, fields, options).toArray();
};

api.count = async ({edvId, query = {}, options = {}}) => {
  // force EDV ID
  query.edvId = database.hash(edvId);
  return database.collections.edvDoc.count(query, options);
};
...
api.update = async ({edvId, doc}) => {
  ...
  try {
    const result = await database.collections.edvDoc.update({
      edvId: record.edvId,
      id: record.id,
...
api.remove = async ({edvId, id}) => {
  ...
  const result = await database.collections.edvDoc.remove(
    {edvId: database.hash(edvId), id: database.hash(id)});
...
api.updateChunk = async ({edvId, docId, chunk}) => {
  ...
  try {
    const result = await database.collections.edvDocChunk.update({
      edvId: record.edvId,
      docId: record.docId,
...
api.removeChunk = async ({edvId, docId, chunkIndex}) => {
  ...
  // TODO: store chunks as files instead of documents
  const result = await database.collections.edvDocChunk.remove({
    edvId: database.hash(edvId), docId: database.hash(docId),
    'chunk.index': chunkIndex
  });
  return result.result.n !== 0;
