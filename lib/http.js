...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  const authorizeDocZcapInvocation = _authorizeZcapInvocation({
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const {config: existingConfig} = await storage.getConfig({id});
      ...
      _reportOperationUsageWithoutWaiting({edvId: id});
    }));
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      // meter operation usage
      _reportOperationUsageWithoutWaiting({edvId});
    }));
...
async function _getRootController({
  ...
  let controller;
  try {
    const record = await storage.getConfig({id: rootInvocationTarget, req});
...
function _reportOperationUsageWithoutWaiting({edvId}) {
  // do not await
