...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const expectedTarget = `${edvId}${docPath}`;
      const expectedRootCapability = `${edvId}/zcaps${docPath}`;
      await _authorize({
        req, expectedTarget, expectedRootCapability, expectedAction: 'write'
      });
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      // check authorization
      const edvId = _getEdvId(req.params.edvId);
      const expectedTarget = `${edvId}/query`;
      const expectedRootCapability = `${edvId}/zcaps/query`;
      await _authorize({
        req, expectedTarget, expectedRootCapability, expectedAction: 'read'
      });
