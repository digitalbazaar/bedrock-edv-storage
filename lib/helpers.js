...
exports.authorize = async ({
  req, expectedTarget, expectedRootCapability, edvConfig, expectedAction
}) => {
  const url = `${config.server.baseUri}${req.originalUrl}`;
  const {method, headers} = req;
  const result = await verifyCapabilityInvocation({
    url, method, headers,
    getInvokedCapability: _createGetInvokedCapability({edvConfig}),
    documentLoader: _createWrappedDocumentLoader({edvConfig, expectedTarget}),
...
exports.generateRootCapability = async ({edvConfig, url}) => {
  // this is a new config
  if(url === `${baseStorageUrl}zcaps/configs` && edvConfig) {
    return {
      '@context': SECURITY_CONTEXT_V2_URL,
      id: url,
      // baseStorageUrl has a trailing slash which does not work here
      invocationTarget: `${baseUri}${routes.edvs}`,
      controller: edvConfig.controller,
      // FIXME: REMOVE?  IS THERE A USE CASE?
      // invoker: edvConfig.invoker,
      // delegator: edvConfig.delegator
    };
  }
...
function _createGetInvokedCapability({edvConfig}) {
  return async ({id, expectedTarget}) => {
    ...
    const zcap = await exports.generateRootCapability({edvConfig, url: id});
    if(zcap) {
      return zcap;
    }
...
function _createWrappedDocumentLoader({edvConfig, expectedTarget}) {
  return async url => {
    if(url.startsWith('did:key:')) {
      return _documentLoader(url);
    }
    ...
    // dynamically generate zcap for root capability if applicable
    const zcap = await exports.generateRootCapability({edvConfig, url});
    if(zcap) {
      return {
        contextUrl: null,
        documentUrl: url,
