...
export async function insert({config}) {
  ...
  try {
    ...
    configRecord = result.ops[0];
    // if no `referenceId` is set, return quickly
    if(config.referenceId === undefined) {
      return configRecord;
    }
  } catch(e) {
  ...
  // config has `referenceID`, so ensure config record is unique
    ...
    try {
      ...
      if(e.name === 'DuplicateError') {
        ...
        try {
          ...
          const {config: {id: existingConfigId}} = await get(
            {id: mappingRecord.configId, _allowPending: true});
          // existing config record found; ensure its no longer marked
          // pending
