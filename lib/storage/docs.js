...
export let DOC_COMPATIBILITY_VERSION = 0;
...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  // FIXME: put this behind a config flag; make doc compatibility mode default
  // to version `0` and call the code below; make new major version change the
  // config flag default to version `1`, which will remove version `0` support
  const tbdConfigFlag = true;
  if(tbdConfigFlag) {
    // ensure legacy `attributes` index is available
    await database.createIndexes([{
    ...
    await _startMigrationToVersion1();
    console.log('DOC_COMPATIBILITY_VERSION', DOC_COMPATIBILITY_VERSION);
  }
});
...
async function _startMigrationToVersion1() {
  ...
  if(migrationRequired) {
    ...
    _migrateToVersion1(cursor).catch(
      error => logger.warn(
        `Failed to perform EDV doc version 1 migration; will try again later.`,
        {error}));
  }

  // if no migration is required, remove old unnecessary index
  if(!migrationRequired) {
    ...
    if(total === version1Docs) {
      // FIXME: put this behind a config flag
      // remove version `0` `attributes` index
      //await collection.dropIndex('attributes');
      DOC_COMPATIBILITY_VERSION = 1;
    }
  }
}
...
async function _migrateToVersion1(cursor) {
  ...
  while(await cursor.hasNext()) {
    ...
    console.log('migrating', record.doc.id);
    if(!await _migrateDocToVersion1(record)) {
      failures++;
    }
  }
